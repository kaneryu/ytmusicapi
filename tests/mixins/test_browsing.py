...
class TestBrowsing:
    async def test_get_home(self, yt, yt_auth):
        result = await yt.get_home()
        assert len(result) >= 2
        result = await yt_auth.get_home(limit=20)
    ...
    async def test_get_artist(self, yt):
        results = await yt.get_artist("MPLAUCmMUZbaYdNH0bEd1PAlAqsA")
        ...
        results = await yt.get_artist("UCLZ7tlKC06ResyDmEStSrOw")  # no album year
        assert len(results) >= 11

    async def test_get_artist_description(self, yt, data_path):
        mock_response = load_json_fixture(data_path / "2026_05_get_artist1.json")

        expected_output = load_json_fixture(data_path / "expected_output" / "2026_05_get_artist1.json")

        with mock.patch("ytmusicapi.YTMusic._send_request", return_value=mock_response):
            result = await yt.get_artist("UCJwGWV914kBlV4dKRn7AEFA")
            assert result["description"] == expected_output["description"]
            assert result["descriptionRuns"] == expected_output["descriptionRuns"]

        mock_response = load_json_fixture(data_path / "2026_05_get_artist2.json")

        expected_output = load_json_fixture(data_path / "expected_output" / "2026_05_get_artist2.json")

        with mock.patch("ytmusicapi.YTMusic._send_request", return_value=mock_response):
            result = await yt.get_artist("UC5CwaMl1eIgY8h02uZw7u8A")
            assert result["description"] == expected_output["description"]
            assert result["descriptionRuns"] == expected_output["descriptionRuns"]

        mock_response = load_json_fixture(data_path / "2026_07_get_artist_hashtag.json")

        expected_output = load_json_fixture(data_path / "expected_output" / "2026_07_get_artist_hashtag.json")

        with mock.patch("ytmusicapi.YTMusic._send_request", return_value=mock_response):
            result = await yt.get_artist("UCRHts_o65erNP48E0j2qYkw")
            assert result["description"] == expected_output["description"]
            assert result["descriptionRuns"] == expected_output["descriptionRuns"]

    async def test_get_artist_two_column_layout(self, yt, data_path):
        ...
        mock_response = load_json_fixture(data_path / "2026_07_get_artist_two_column.json")

        with mock.patch("ytmusicapi.YTMusic._send_request", return_value=mock_response):
            result = await yt.get_artist("UCTestChannelId")
    ...
    async def test_get_artist_shows(self, yt_oauth):
        # with audiobooks - only with authentication
        results = await yt_oauth.get_artist("UCyiY-0Af0O6emoI3YvCEDaA")
        assert len(results["shows"]["results"]) == 10

        results = await yt_oauth.get_artist_albums(
            results["shows"]["browseId"], results["shows"]["params"]
        )
        assert len(results) == 100

    async def test_get_artist_albums(self, yt):
        artist = await yt.get_artist("UCAeLFBCQS7FvI8PvBrWvSBg")
        results = await yt.get_artist_albums(artist["albums"]["browseId"], artist["albums"]["params"])
        assert all("artists" not in result for result in results)  # artist info is omitted from the results
        assert len(results) == 100
        results = await yt.get_artist_albums(artist["singles"]["browseId"], artist["singles"]["params"])
        assert len(results) == 100

        results_unsorted = await yt.get_artist_albums(
            artist["albums"]["browseId"], artist["albums"]["params"], limit=None
        )
        assert len(results_unsorted) >= 300

        results_sorted = await yt.get_artist_albums(
        ...
        with pytest.raises(ValueError, match="Invalid order"):
            await yt.get_artist_albums(
                artist["albums"]["browseId"], artist["albums"]["params"], order="order"
            )

    async def test_get_user(self, yt):
        results = await yt.get_user("UC44hbeRoCZVVMVg5z0FfIww")
        assert len(results) == 3

    async def test_get_user_playlists(self, yt, yt_auth):
        channel = "UCPVhZsC2od1xjGhgEc2NEPQ"  # Vevo playlists
        user = await yt_auth.get_user(channel)
        results = await yt_auth.get_user_playlists(channel, user["playlists"]["params"])
        assert len(results) > 100

        results_empty = await yt.get_user_playlists(channel, user["playlists"]["params"])
        assert len(results_empty) == 0

    async def test_get_user_videos(self, yt, yt_auth):
        channel = "UCus8EVJ7Oc9zINhs-fg8l1Q"  # Turbo
        user = await yt_auth.get_user(channel)
        results = await yt_auth.get_user_videos(channel, user["videos"]["params"])
        assert len(results) > 100

        results_empty = await yt.get_user_videos(channel, user["videos"]["params"])
        assert len(results_empty) == 0

    async def test_get_album_browse_id(self, yt, sample_album):
        warnings.filterwarnings(action="ignore", category=DeprecationWarning)
        browse_id = await yt.get_album_browse_id("OLAK5uy_nMr9h2VlS-2PULNz3M3XVXQj_P3C2bqaY")
        assert browse_id == sample_album

    async def test_get_album_browse_id_issue_470(self, yt):
        escaped_browse_id = await yt.get_album_browse_id("OLAK5uy_nbMYyrfeg5ZgknoOsOGBL268hGxtcbnDM")
        assert len(escaped_browse_id) == 17

    async def test_get_album_2024(self, yt, data_path):
        mock_response = load_json_fixture(data_path / "2024_03_get_album.json")
        with mock.patch("ytmusicapi.YTMusic._send_request", return_value=mock_response):
            album = await yt.get_album("MPREabc")
    ...
    async def test_get_album(self, yt, yt_auth, sample_album):
        album = await yt_auth.get_album(sample_album)
        assert len(album) >= 9
        ...
        album = await yt.get_album("MPREb_BQZvl3BFGay")
        assert album["audioPlaylistId"] is not None
        assert len(album["tracks"]) == 7
        assert len(album["tracks"][0]["artists"]) == 1
        album = await yt.get_album("MPREb_7HdnOQMfJ3w")
        assert album["likeStatus"] is not None
        assert album["audioPlaylistId"] is not None
        assert len(album["tracks"][0]["artists"]) == 2
        album = await yt.get_album("MPREb_G21w42zx0qJ")  # album with track (#13) disabled/greyed out
        assert album["likeStatus"] is not None
        assert album["audioPlaylistId"] is not None
        assert album["tracks"][12]["trackNumber"] is None
        assert not album["tracks"][12]["isAvailable"]

    async def test_get_album_with_description_contain_link(self, yt, data_path):
        mock_response = load_json_fixture(data_path / "2026_05_get_album.json")

        expected_output = load_json_fixture(data_path / "expected_output" / "2026_05_get_album.json")

        with mock.patch("ytmusicapi.YTMusic._send_request", return_value=mock_response):
            browse_id = await yt.get_album_browse_id("OLAK5uy_kW9hN-oBmekJ06jhhfStpwRd5pcRKIztY")
            result = await yt.get_album(browse_id)
            assert result["description"] == expected_output["description"]
            assert result["descriptionRuns"] == expected_output["descriptionRuns"]

    async def test_get_album_errors(self, yt):
        with pytest.raises(Exception, match="Invalid album browseId"):
            await yt.get_album("asdf")

    async def test_get_album_without_artist(self, yt):
        album = await yt.get_album("MPREb_n1AxZ9F8rF7")  # soundtrack album with no artist info
        assert album["artists"] is None
        assert album["audioPlaylistId"] is not None
        assert len(album["tracks"]) == 11

    async def test_get_album_other_versions(self, yt, yt_oauth):
        # Eminem - Curtain Call: The Hits (Explicit Variant)
        album = await yt_oauth.get_album("MPREb_LQCAymzbaKJ")
        ...
        # Cassö & RAYE - Prada
        album = await yt.get_album("MPREb_of3qfisa0yU")
    ...
    async def test_get_song(self, config, yt, yt_oauth, sample_video):
        song = await yt_oauth.get_song(config["uploads"]["private_upload_id"])  # private upload
        assert len(song) == 5
        song = await yt.get_song(sample_video)
    ...
    async def test_get_song_related_content(self, yt_oauth, sample_video):
        song = await yt_oauth.get_watch_playlist(sample_video)
        song = await yt_oauth.get_song_related(song["related"])
        assert len(song) >= 5
        assert all(
    ...
    async def test_get_lyrics(self, config, yt, sample_video):
        playlist = await yt.get_watch_playlist(sample_video)
        # test normal lyrics
        lyrics_song = await yt.get_lyrics(playlist["lyrics"])
        ...
        lyrics_song = await yt.get_lyrics(playlist["lyrics"], timestamps=True)
        assert lyrics_song is not None
    ...
    async def test_get_signatureTimestamp(self, yt):
        signature_timestamp = await yt.get_signatureTimestamp()
        assert signature_timestamp is not None

    async def test_set_tasteprofile(self, yt_brand):
        with pytest.raises(YTMusicUserError):
            await yt_brand.set_tasteprofile(["test", "test2"])
        taste_profile = await yt_brand.get_tasteprofile()
        assert await yt_brand.set_tasteprofile(list(taste_profile)[:1], taste_profile) is None

    async def test_get_tasteprofile(self, yt, yt_oauth):
        result = await yt_oauth.get_tasteprofile()
        assert len(result) >= 0

    async def test_get_search_suggestions(self, yt, yt_brand, yt_auth):
        result = await yt.get_search_suggestions("fade")
        assert len(result) >= 0

        result = await yt.get_search_suggestions("fade", detailed_runs=True)
        assert len(result) >= 0

        # add search term to history
        first_pass = await yt_brand.search("b")
        assert len(first_pass) > 0

        # get results
        results = await yt_auth.get_search_suggestions("b", detailed_runs=True)
        assert len(results) > 0
        assert any(not item["fromHistory"] for item in results)

    async def test_get_song_credits(self, yt, sample_credits):
        credits = await yt.get_song_credits(sample_credits)
