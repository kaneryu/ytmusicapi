...
class TestPodcasts:
    async def test_get_channel(self, config, yt):
        podcast_id = config["podcasts"]["channel_id"]
        channel = await yt.get_channel(podcast_id)
        assert len(channel["episodes"]["results"]) == 10
        assert len(channel["podcasts"]["results"]) >= 5

    async def test_get_channel_episodes(self, config, yt_oauth):
        channel_id = config["podcasts"]["channel_id"]
        channel = await yt_oauth.get_channel(channel_id)
        channel_episodes = await yt_oauth.get_channel_episodes(channel_id, channel["episodes"]["params"])
        assert len(channel_episodes) >= 150
        assert len(channel_episodes[0]) == 9

    async def test_get_podcast(self, config, yt, yt_brand):
        podcast_id = config["podcasts"]["podcast_id"]
        podcast = await yt.get_podcast(podcast_id)
        assert len(podcast["episodes"]) == 100
        assert not podcast["saved"]
        assert podcast["thumbnails"]

        podcast = await yt_brand.get_podcast(podcast_id, limit=None)
        assert len(podcast["episodes"]) > 100
        assert podcast["saved"]

    async def test_many_podcasts(self, yt):
        results = await yt.search("europe", filter="podcasts")
        for result in results:
            if result["browseId"] is None:
                continue  # search can surface non-podcast results even with the podcasts filter
            podcast = await yt.get_podcast(result["browseId"])
            assert len(podcast) > 0

    async def test_get_episode(self, config, yt, yt_brand):
        episode_id = config["podcasts"]["episode_id"]
        episode = await yt.get_episode(episode_id)
        assert len(episode["description"]) >= 20
        assert not episode["saved"]
        assert episode["playlistId"] is not None
        assert episode["thumbnails"]

        episode = await yt_brand.get_episode(episode_id)
        assert episode["saved"]

    async def test_many_episodes(self, yt):
        results = await yt.search("europe", filter="episodes")
        episodes = []
        for result in results:
            try:
                episode = await yt.get_episode(result["videoId"])
    ...
    async def test_get_episodes_playlist(self, yt_brand):
        playlist = await yt_brand.get_episodes_playlist()
