...
class BrowsingMixin(MixinProtocol):
    ...
    async def get_song_album_id(self, videoId: str) -> str | None:
        ...
        """
        response = await self._send_request("next", {"videoId": videoId, "index": 0, "watchNextType": "WATCH_NEXT_TYPE_SKIP_VIDEO"})
        ...
        menu_items = nav(response, [*SINGLE_COLUMN_WATCH_NEXT_TAB, *AUTOPLAY_ITEM_LIST, 0, "playlistPanelVideoRenderer", *MENU_ITEMS], True) or []
        if not menu_items:
            return None
